...
public class Exam0120 {
  public static void main(String[] args) {
    ...
    Score s; 
    // 이 선언은 설계도에 따라 메모리를 준비시키는 명령이 아닌, Score 설계도에 따라 만든 메모리의 주소를 저장할 변수를 만드는 명령.
    // => 메모리의 주소를 저장하는 변수 : reference
    // int와 같은 primitive data type인 경우 위와 같이 그냥 변수를 선언하면 되지만, 클래스 문법으로 설계한 메모리는 이런 방식으로 만들 수 없다.
    
    // class 문법으로 설계한 메모리 준비하기
    // => new class_name();
    
    // => 확보된 메모리를 사용하려면 주소를 보관할 필요가 있다.
    // => 위에서 준비한 레퍼런스에 주소를 보관하는 것.
    s = new Score(); // Score 설계도에 따라 메모리(Instance)를 확보하고,
    // 그 메모리의 주소를 s에 저장한다.   
