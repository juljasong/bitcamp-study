...
public interface Computer {
    ...
    //But, 인터페이스에 선언하는 메서드는 기본이 추상 메서드이기 때문에 그 인터페이스를 구현하는 
    //클래스는 반드시 해당 메서드를 정의해야 한다.
    //그러나 default 메서드는 이미 구현되어 있어 클래스에서 반드시 정의할 필요가 없다.
    //즉, 강제로 정의하게 만들 수 없는 것이 문제이다.
    //기존에 구축된 인터페이스에 새 규칙 추가할 때, 기존 프로젝트에 영향을 끼치지 않기 위해 사용하는 것
}
