...
public class Exam0210 {
  public static void main(String[] args) throws Exception {
    ...
    System.out.println(s1.compareTo("Helli")); // 6 (알파벳 순으로 Hello 보다 앞)
    System.out.println(s1.compareTo("Hello")); // 0
    System.out.println(s1.compareTo("Hellu")); // -6 (알파벳 순으로 Hello 보다 뒤)
    ...
    System.out.println(s2); // 새로 문자열을 만든다. => Hello, world!
    ...
    for (int i = 0; i < bytes.length; i++)
      System.out.printf("%x,", bytes[i]); // 41,42,43,ea,b0,80,ea,b0,81,
    System.out.println();

    // 다른 인스턴스 메서드를 사용하여 바이트 배열을 추출해보자.
    bytes = s3.getBytes("EUC-KR");
    for (int i = 0; i < bytes.length; i++)
      System.out.printf("%x,", bytes[i]); // 41,42,43,b0,a1,b0,a2,
    System.out.println();
    ...
    // => 형식을 갖춘 문자열을 만들기
    String s4 = String.format("%s님 반갑습니다", "홍길동"); // 홍길동님 반갑습니다
    ...
    System.out.println(s6); // (문자열)true
    System.out.println(s7); // (문자열)3.14
    System.out.println(s8); // (문자열)100
